...
- (void) webView:(WKWebView *) webView decidePolicyForNavigationAction:(nonnull WKNavigationAction *)navigationAction decisionHandler:(nonnull void (^)(WKNavigationActionPolicy))decisionHandler{
    if ([navigationAction.request.URL.host isEqual: self.originURL.host]) {
        decisionHandler(WKNavigationActionPolicyAllow);
        return;
    } else if ([navigationAction.request.URL.scheme isEqual:@"ytplayer"]) {
 ...
 *
 * @param state |YTPlayerState| parameter.
...
- (BOOL)loadWithPlayerParams:(NSDictionary *)additionalPlayerParams {
    ...
    NSError *error = nil;
    __strong NSString *path = [[NSBundle bundleForClass:[YTPlayerView class]] pathForResource:@"YTPlayerView-iframe-player"
                                                                                       ofType:@"html"
                                                                                  inDirectory:@"Assets"];
...
- (NSString *)stringFromEvaluatingJavaScript:(NSString *)jsToExecute {
    ...
    [self.webView evaluateJavaScript: jsToExecute completionHandler:^(NSString *result , NSError *error){
        data = result;
        finished = YES;
    }];
    while(!finished){
